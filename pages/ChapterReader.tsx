...
const ChapterReader: React.FC = () => {
  ...
  useEffect(() => {
    if (currentNum > 0) {
      localStorage.setItem('nanomachine_last_chapter', String(currentNum));
    }
  }, [currentNum]);
  ...
  return (
    <div className="bg-gray-100 dark:bg-[#121212] min-h-screen flex flex-col">
      <SEOHead
        title={`Nano Machine Manga Chapter ${chapter.number} - Read Online`}
        description={`Read Nano Machine Manga Chapter ${chapter.number}: ${chapter.title} online in high quality free. Official English scans available.`}
        schema={{
          "@context": "https://schema.org",
          "@type": "ComicIssue",
          "headline": `Nano Machine Chapter ${chapter.number}: ${chapter.title}`,
        ...
        <div className="max-w-7xl mx-auto px-3 h-14 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
              <span className="text-[10px] text-bb-blue font-bold uppercase tracking-wider leading-none mb-0.5">Nano Machine</span>
              <select value={chapter.number} onChange={e => navigate(`/chapter/${e.target.value}`)} className="font-bold text-sm dark:text-white bg-transparent focus:outline-none cursor-pointer truncate max-w-[160px] md:max-w-[280px] dark:bg-gray-900" title="Jump to chapter">
                {[...chapters].sort((a, b) => b.number - a.number).map(ch => (
                  <option key={ch.number} value={ch.number} className="dark:bg-gray-900">Ch. {ch.number}{ch.title && ch.title !== `Chapter ${ch.number}` ? ` — ${ch.title.length > 30 ? ch.title.substring(0, 30) + '…' : ch.title}` : ''}</option>
                ))}
              </select>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            <button disabled={!prevChapter} onClick={() => prevChapter && handleNav(prevChapter.number)} className="flex items-center gap-1 px-2 py-1.5 disabled:opacity-30 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md dark:text-white transition-colors text-xs font-bold" title="Previous chapter (←)">
              <ChevronLeft size={16} /><span className="hidden sm:block">Prev</span>
            </button>
            <button disabled={!nextChapter} onClick={() => nextChapter && handleNav(nextChapter.number)} className="flex items-center gap-1 px-2 py-1.5 disabled:opacity-30 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md dark:text-white transition-colors text-xs font-bold" title="Next chapter (→)">
              <span className="hidden sm:block">Next</span><ChevronRight size={16} />
            </button>
          </div>
        </div>
      </div>
      ...
      {showScrollTop && (
        <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="fixed bottom-6 right-5 z-50 w-10 h-10 bg-bb-blue hover:bg-blue-700 text-white rounded-full shadow-xl flex items-center justify-center transition-all hover:scale-110" title="Back to top">
      ...
      <div className="flex-1 pt-16">
        <h1 className="sr-only">Read Nano Machine Chapter {chapter.number}: {chapter.title}</h1>
        {chapter.pages.length > 0 && (
          <div className="max-w-4xl mx-auto px-4 py-4">
            <Breadcrumbs items={[
              { label: 'Home', path: '/' },
              { label: 'Nano Machine', path: '/manga' },
          ...
          <div className="max-w-4xl mx-auto bg-white dark:bg-black shadow-2xl min-h-screen">
            {chapter.pages.map((pageUrl, idx) => (
              <img
                key={idx}
                src={pageUrl}
                alt={`Nano Machine Chapter ${chapter.number} Page ${idx + 1}`}
        ...
        <div className="max-w-4xl mx-auto mt-12 px-4 pb-20">
          <div className="bg-[#1a1a1a] border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-white/5 bg-white/5">
              ...
              <button
                onClick={() => navigator.share?.({ title: `Nano Machine Chapter ${chapter.number}`, url: window.location.href })}
